...
class MainWindow(QTabWidget):
    ...
    ***上传图片***
    ...
    def upload_img(self):
        ...
        if fileName:
            ...
            save_path = osp.join("images/tmp", "upload." + suffix)
            shutil.copy(fileName, save_path)
            im0 = cv2.imread(save_path)
            resize_scale = self.output_size / im0.shape[0]
            im0 = cv2.resize(im0, (0, 0), fx=resize_scale, fy=resize_scale)
            cv2.imwrite("images/tmp/result.jpg", im0)
            self.img2predict = fileName
            self.left_img.setPixmap(QPixmap("images/tmp/result.jpg"))
            #左侧图片添加上之后将右侧图片重新加载下
            self.right_img.setPixmap(QPixmap("images/UI/right.png"))
    ...
    ***选择权重文件***
    ...
    def chpath(self):
        ...
        if fileName !='':
            self.model = self.model_load(weights=fileName,
                                     device=self.device)
            self.label_patch.setText(fileName)
        return fileName
    ...
    def chpath2(self):
        ...
        if x==1:
            print("切换到CPU进行计算")
            self.model = self.model_load(weights="best1.pt",
                                         device='cpu')
            x = 2
            self.about_title2.setText('当前在使用CPU计算')
        else:
            print("切换到CPU进行计算")
            self.model = self.model_load(weights="best1.pt",
                                         device='0')
            x = 1
            self.about_title2.setText('当前在使用GPU计算')
        return
    '''
    ***检测图片***
    '''
    def detect_img(self):
        ...
        print("图片路径是：",source)
        if source == "":
            QMessageBox.warning(self, "Warning", "请先上传图片再进行检测")
        else:
            source = str(source)
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    im0 = cv2.resize(im0, (0, 0), fx=resize_scale, fy=resize_scale)
                    cv2.imwrite("images/tmp/result.jpg", im0)
                    self.right_img.setPixmap(QPixmap("images/tmp/result.jpg"))
    ...
    def closeEvent(self, event):
        reply = QMessageBox.question(self,'退出',"确定要关闭吗?",QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
    ...
    def detect_vid(self):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                cv2.imwrite("images/tmp/result.jpg", frame_resized)
                self.vid_img.setPixmap(QPixmap("images/tmp/result.jpg"))
                # self.vid_img
                # if view_img:
                # cv2.imshow(str(p), im0)
...
if __name__ == "__main__":
    x = 1  # 切换cpugpu的计数器，如果是1的话就是GPU,如果是2就是CPU
